...
if __name__ == '__main__':
    # 加载模型
    #model = YOLO(r'yolov8-seg.yaml')  # 不使用预训练权重训练，仅加载架构，重新开始训练
    model = YOLO('weights/original_weigths/yolov8n-seg.pt')  # load a pretrained model (recommended for training)
    # model = YOLO('yolov8n.yaml').load('yolov8n.pt')  # build from YAML and transfer weights
    # 训练参数 ----------------------------------------------------------------------------------------------
    model.train(
        #data=r'E:\master files\yellow_flower_detection\data\YOLOV8-seg_for_yellow\data.yaml',
        data=r'E:\master files\yellow_flower_detection\data\labelme_work\dataset.yaml',
        ...
        project='train',  # (str, optional) 项目名称
